-- Test script for e-filing logging functionality with correct action types
-- This will insert test records to verify the system is working

-- Insert test login actions (using correct constants from EFILING_ACTION_TYPES)
...
(
    NULL, '1', 'USER_LOGOUT', 'Test user logged out of e-filing system', NOW(), NOW(),
    'efiling_user', 1, 'Test User', 'test@example.com', 'auth', 'e-filing_logout',
    '{"method": "session_timeout", "userType": "efiling_user"}', '127.0.0.1', 'Test Browser'
);

-- Verify the data was inserted
SELECT 
    id, user_id, action_type, description, user_name, user_email, 
    entity_type, entity_name, timestamp, file_id
