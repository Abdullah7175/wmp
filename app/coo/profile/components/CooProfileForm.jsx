...
export default function CooProfileForm() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await fetch('/api/coo/profile');
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/coo/profile/password', {
        method: 'PUT',
        headers: {
          ...
          <form onSubmit={handleProfileSubmit} className="space-y-6">
            ...
            <div className="flex justify-end pt-6 border-t border-gray-200">
              <button
                type="submit"
                disabled={isLoading}
                className="px-6 py-2 text-sm font-medium text-white bg-green-600 rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
              >
