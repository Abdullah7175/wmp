...
export async function GET(request) {
    ...
    try {
        if (id) {
            ...
            let dataQuery = 'SELECT id, name, email, contact_number, role, CASE WHEN image IS NOT NULL THEN CONCAT(\'http://119.30.113.18:3000\', image) ELSE NULL END as image, created_date FROM users';
            let whereClauses = [];
            let params = [];
            let paramIdx = 1;
            if (filter) {
