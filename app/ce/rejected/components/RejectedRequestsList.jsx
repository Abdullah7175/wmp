  ...
  const fetchRejectedRequests = async () => {
    try {
      ...
      if (data.success) {
        ...
        const rejectedRequests = data.data.filter(request => 
          request.ce_approval_status === 'not_approved'
        );
        setRequests(rejectedRequests);
      } else {
        toast({
          title: "Error",
          description: data.message || "Failed to fetch rejected requests",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-lg">Loading rejected requests...</div>
      </div>
    );
  }
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-center text-gray-500">
              ...
              <p>You haven't rejected any requests yet.</p>
            </div>
          </CardContent>
        </Card>
      ) : (
        requests.map((request) => (
          <Card key={request.id} className="hover:shadow-md transition-shadow border-red-200">
            ...
            <CardContent>
              ...
              {/* CE Rejection Details */}
              <div className="mb-4 p-3 bg-red-50 rounded border border-red-200">
                <div className="text-sm font-medium text-red-800 mb-2">Your Rejection</div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm text-gray-600">
                    Rejected on: {request.ce_approval_date ? formatDate(request.ce_approval_date) : 'N/A'}
                  </span>
                </div>
                {request.ce_comments && (
                  <p className="text-sm text-gray-700">
