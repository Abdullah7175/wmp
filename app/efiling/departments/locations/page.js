...
import { Loader2, Map, RefreshCw, Save, X, Trash2, Filter } from "lucide-react";
...
export default function DepartmentLocationsPage() {
    ...
    const loadReferenceData = async () => {
        try {
            const [deptRes, zoneRes, districtRes, townRes, divisionRes] = await Promise.all([
    ...
    const fetchLocations = async () => {
        ...
        try {
            const url = departmentFilter !== "all"
                ? `/api/efiling/departments/locations?department_id=${departmentFilter}`
                : "/api/efiling/departments/locations";
            ...
            toast({
                title: "Error",
                description: "Unable to load department locations",
                variant: "destructive",
            });
        } finally {
            setLoading(false);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await fetch("/api/efiling/departments/locations", {
            ...
            toast({
                title: "Success",
                description: editingId ? "Location mapping updated" : "Location mapping created",
            });
            resetForm();
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const res = await fetch(`/api/efiling/departments/locations?id=${id}`, { method: "DELETE" });
            if (!res.ok) {
                const error = await res.json().catch(() => ({}));
                throw new Error(error?.error || "Failed to delete mapping");
            }
            toast({ title: "Deleted", description: "Location mapping removed" });
        ...
        <div className="container mx-auto px-4 py-6 space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2">
                        <Map className="w-7 h-7 text-blue-600" /> Department Location Mappings
                    </h1>
                    <p className="text-gray-600">
                        Link departments to zones, districts, towns, and divisions to drive routing and visibility.
                    </p>
                </div>
            ...
            {canManage ? (
                ...
                <Card>
                    <CardContent className="py-6 text-gray-600">
                        You have read-only access to department location mappings.
                    </CardContent>
                </Card>
            )}
            ...
            <Card>
                <CardHeader className="flex items-center justify-between">
                    <CardTitle>Mappings ({locations.length})</CardTitle>
                    <div className="flex items-center gap-2 text-sm text-gray-500">
                        <Filter className="w-4 h-4" />
                        {departmentFilter === "all" ? "All departments" : "Filtered by department"}
                    </div>
                </CardHeader>
                <CardContent>
                    {loading ? (
                        ...
                        <div className="text-center text-gray-500 py-10">
                            No geographic mappings configured.
                        </div>
                    ) : (
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {locations.map((loc) => (
                                        <TableRow key={loc.id}>
                                            <TableCell>
                                                <div className="font-medium">{loc.department_name || "-"}</div>
                                                <div className="text-xs text-gray-500">{loc.department_code}</div>
                                            </TableCell>
