...
export default function EditEfilingDepartment() {
    ...
    // Load department data and dependencies
    useEffect(() => {
        if (params.id) {
            loadDepartmentData();
            loadDependencies();
    ...
    const loadDepartmentData = async () => {
        try {
            const response = await fetch(`/api/efiling/departments?id=${params.id}`);
            if (response.ok) {
    ...
    const loadParentDepartments = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const filteredData = data.filter(dept => dept.id != params.id);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                toast({
                    title: "Success",
                    description: "E-filing department updated successfully!",
                });
            ...
            toast({
                title: "Error",
                description: error.message || "Failed to update e-filing department.",
                variant: "destructive",
            });
        } finally {
            setLoading(false);
    ...
    return (
        <div className="container mx-auto py-6 px-4">
            <div className="flex items-center gap-4 mb-6">
                ...
                <h1 className="text-2xl font-bold">Edit E-Filing Department</h1>
            </div>
