...
import { Loader2, MapPin, RefreshCw, Save, Trash2, X, Filter } from "lucide-react";
...
export default function RoleLocationsPage() {
    ...
    const loadReferenceData = async () => {
        try {
            const [roleRes, zoneRes, districtRes, townRes, divisionRes] = await Promise.all([
    ...
    const fetchLocations = async () => {
        ...
        try {
            const url = roleFilter !== "all"
                ? `/api/efiling/roles/locations?role_id=${roleFilter}`
                : "/api/efiling/roles/locations";
            const res = await fetch(url);
            if (!res.ok) throw new Error("Failed to load role locations");
            ...
            toast({
                title: "Error",
                description: "Unable to load role locations",
                variant: "destructive",
            });
        } finally {
            setLoading(false);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const response = await fetch("/api/efiling/roles/locations", {
            ...
            toast({
                title: "Success",
                description: editingId ? "Role mapping updated" : "Role mapping created",
            });
            resetForm();
    ...
    const handleEdit = (location) => {
        ...
        setForm({
            role_id: location.role_id?.toString() || "",
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const res = await fetch(`/api/efiling/roles/locations?id=${id}`, { method: "DELETE" });
            if (!res.ok) {
                const error = await res.json().catch(() => ({}));
                throw new Error(error?.error || "Failed to delete mapping");
            }
            toast({ title: "Deleted", description: "Role mapping removed" });
        ...
        <div className="container mx-auto px-4 py-6 space-y-6">
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2">
                        <MapPin className="w-7 h-7 text-blue-600" /> Role Location Mappings
                    </h1>
                    <p className="text-gray-600">
                        Restrict each role to the correct zone, division, district, and town.
                    </p>
                </div>
            ...
            {canManage ? (
                ...
                <Card>
                    <CardContent className="py-6 text-gray-600">
                        You have read-only access to role location mappings.
                    </CardContent>
                </Card>
            )}
            ...
            <Card>
                <CardHeader className="flex items-center justify-between">
                    <CardTitle>Role Mappings ({locations.length})</CardTitle>
                    <div className="flex items-center gap-2 text-sm text-gray-500">
                        <Filter className="w-4 h-4" />
                        {roleFilter === "all" ? "All roles" : "Filtered by role"}
                    </div>
                </CardHeader>
                <CardContent>
                    {loading ? (
                        ...
                        <div className="text-center text-gray-500 py-10">
                            No role location mappings configured.
                        </div>
                    ) : (
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {locations.map((loc) => (
                                        <TableRow key={loc.id}>
                                            <TableCell>
                                                <div className="font-medium">{loc.role_name || "-"}</div>
                                                <div className="text-xs text-gray-500">{loc.role_code}</div>
                                            </TableCell>
