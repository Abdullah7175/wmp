...
// Helper functions to safely resolve configuration and recover from dotenv-expand truncation
export function getWhatsAppSecretKey() {
    let key = process.env.WHATSAPP_SECRET_KEY;
    if (key) {
        // Strip wrapping quotes if present
        key = key.replace(/^['"]|['"]$/g, '').trim();
        // Unescape escaped dollar sign if present (from .env kw\$0126001)
        key = key.replace(/\\\$/g, '$');
    }
    // Critical fix: If dotenv-expand or shell expanded "$0126001" as an empty variable,
    // "kw$0126001" becomes truncated to just "kw" (length: 2).
    // Automatically recover the full secret token.
    if (!key || key === 'kw' || key.length < 5) {
        key = 'kw$0126001';
    }
    return key;
}
...
export function getWhatsAppApiUrl() {
    return process.env.WHATSAPP_API_URL || 'http://erp.bizintel.co:8005/api/send-json';
}
...
export async function sendWhatsAppMessage(phoneNumber, message) {
    try {
        ...
        const WHATSAPP_SECRET_KEY = getWhatsAppSecretKey();
 ...
 * @returns {Promise<{success: boolean, message?: string, error?: string}>}
 */
export async function sendOTPViaWhatsApp(phoneNumber, otpCode, userName = 'User', purpose = 'esignature') {
    // Dynamic action description based on purpose
    const actionText = purpose === 'close_file' ? 'file closure verification' : 'e-signature verification';

    const message = `Dear ${userName},\n\nYour OTP for ${actionText} is: ${otpCode}\n\nThis code is valid for 120 seconds. Please do not share this code with anyone.\n\nThank you,\nE-Filing System`;
