...
export async function sendWhatsAppMessage(phoneNumber, message) {
    try {
        ...
        console.log('Sending WhatsApp message:', {
            ...
            token: WHATSAPP_SECRET_KEY ? '***configured***' : 'missing',
        ...
        if (!response.ok) {
            ...
            if (response.status === 401) {
                throw new Error(`WhatsApp API authentication failed (401). Please verify: 1) The secret key is correct (${WHATSAPP_SECRET_KEY.substring(0, 3)}***), 2) The API URL is correct (${WHATSAPP_API_URL}), 3) The API expects the format being sent (${useJsonFormat ? 'JSON' : 'form-data'}). Error: ${errorText.substring(0, 200)}`);
            }
