...
export async function canMarkFile(client, fileId, userId) {
    ...
    const file = fileRes.rows[0];
    const creatorId = Number(file.created_by);
    const assignedTo = file.assigned_to != null ? Number(file.assigned_to) : null;
    const currentUserId = Number(userId);

    // Current assignee can always mark/forward (type-safe compare)
    if (assignedTo !== null && assignedTo === currentUserId) {
        return true;
    }

    // Creator can mark when file is with them / within team / returned
    ...
    if (file.workflow_state_id) {
        ...
        if (stateRes.rows.length > 0) {
            ...
            // External flow: only the current assignee (already handled above)
            ...
            if (state.is_within_team || state.current_state === 'TEAM_INTERNAL') {
                const userIsTeamMember = await isTeamMember(client, creatorId, currentUserId);
                return isCreator || userIsTeamMember;
            }
