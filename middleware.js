...
function getDashboardForUser(session) {
    ...
    if ((session.user?.role === 24 || session.user?.role === 8) && session.user?.userType === "user") return "/ceo";
    // Route COO to COO portal
    if ((session.user?.role === 26 || session.user?.role === 6) && session.user?.userType === "user") return "/coo";
...
export async function middleware(req) {
    ...
    try {
        ...
        if (!sessionCookie) {
            ...
            const publicRoutes = ['/login', '/elogin', '/', '/unauthorized', '/public'];
